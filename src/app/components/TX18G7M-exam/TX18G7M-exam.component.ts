...
import * as problemsData from "src/assets/problems/TX18G7M/TX18G7M-problems.json";
...
@Component({
    selector: 'app-TX18G7M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX18G7M-exam.component.css']
})

@Injectable()
export class TX18G7MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX18G7M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
