...
@Component({
    selector: 'app-PA21G6M-key',
    templateUrl: '../template-key/template-key.component.html',
    styleUrls: ['./PA21G6M-key.component.css']
})

@Injectable()
export class PA21G6MKeyComponent implements OnInit {
    title = 'More Problems';
    ...
    exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = PA21G6MProblems;
