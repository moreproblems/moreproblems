...
import * as problemsData from "src/assets/problems/PA16G4S/PA16G4S-problems.json";
...
@Component({
    selector: 'app-PA16G4S-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA16G4S-exam.component.css']
})

@Injectable()
export class PA16G4SExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA16G4S_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
