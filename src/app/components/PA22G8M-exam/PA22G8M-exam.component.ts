...
import * as problemsData from "src/assets/problems/PA22G8M/PA22G8M-problems.json";
...
@Component({
    selector: 'app-PA22G8M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA22G8M-exam.component.css']
})

@Injectable()
export class PA22G8MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA22G8M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
