...
export class TemplateExamComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    // for (let i: number = 0; i < this.exam_length; i++) {
    //     this.ordered_dump[+Object.keys(this.ordered_dump)[i]] = this.e_dump_dict[this.key][+Object.keys(this.ordered_dump)[i]];
    // }
    if (this.exam_attribute_dump[this.key].Parts.length == 0) {
      this.ordered_dump = this.e_dump_dict[this.key];
