...
import * as problemsData from "src/assets/problems/PA16G6M/PA16G6M-problems.json";
...
@Component({
    selector: 'app-PA16G6M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA16G6M-exam.component.css']
})

@Injectable()
export class PA16G6MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA16G6M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
