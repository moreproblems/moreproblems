...
import * as problemsData from "src/assets/problems/TX19G8M/TX19G8M-problems.json";
...
@Component({
    selector: 'app-TX19G8M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX19G8M-exam.component.css']
})

@Injectable()
export class TX19G8MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX19G8M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
