...
import * as problemsData from "src/assets/problems/TX21G8M/TX21G8M-problems.json";
...
@Component({
    selector: 'app-TX21G8M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX21G8M-exam.component.css']
})

@Injectable()
export class TX21G8MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX21G8M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
    ...
    completeExam() {
        ...
        else if (this.number_correct >= 27) {
            this.performance_level = "Meets Grade Level Performance";
        }
        else if (this.number_correct >= 19) {
