...
import * as problemsData from "src/assets/problems/PA15G8M/PA15G8M-problems.json";
...
@Component({
    selector: 'app-PA15G8M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA15G8M-exam.component.css']
})

@Injectable()
export class PA15G8MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA15G8M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
