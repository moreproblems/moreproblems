...
import * as problemsData from "src/assets/problems/TX22G6M/TX22G6M-problems.json";
...
@Component({
    selector: 'app-TX22G6M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX22G6M-exam.component.css']
})

@Injectable()
export class TX22G6MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX22G6M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
