...
import * as problemsData from "src/assets/problems/PA18G3M/PA18G3M-problems.json";
...
@Component({
    selector: 'app-PA18G3M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA18G3M-exam.component.css']
})

@Injectable()
export class PA18G3MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA18G3M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
