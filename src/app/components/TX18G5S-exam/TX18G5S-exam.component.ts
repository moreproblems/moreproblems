...
import * as problemsData from "src/assets/problems/TX18G5S/TX18G5S-problems.json";
...
@Component({
    selector: 'app-TX18G5S-exam',
    templateUrl: './TX18G5S-exam.component.html',
    styleUrls: ['./TX18G5S-exam.component.css']
})

@Injectable()
export class TX18G5SExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX18G5S_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
