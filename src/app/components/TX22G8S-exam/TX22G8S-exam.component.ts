...
import * as problemsData from "src/assets/problems/TX22G8S/TX22G8S-problems.json";
...
@Component({
    selector: 'app-TX22G8S-exam',
    templateUrl: './TX22G8S-exam.component.html',
    styleUrls: ['./TX22G8S-exam.component.css']
})

@Injectable()
export class TX22G8SExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX22G8S_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
