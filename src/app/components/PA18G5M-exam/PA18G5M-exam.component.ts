...
import * as problemsData from "src/assets/problems/PA18G5M/PA18G5M-problems.json";
...
@Component({
    selector: 'app-PA18G5M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA18G5M-exam.component.css']
})

@Injectable()
export class PA18G5MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA18G5M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
