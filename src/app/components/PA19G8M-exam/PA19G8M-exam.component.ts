...
import * as problemsData from "src/assets/problems/PA19G8M/PA19G8M-problems.json";
...
@Component({
    selector: 'app-PA19G8M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA19G8M-exam.component.css']
})

@Injectable()
export class PA19G8MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA19G8M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
