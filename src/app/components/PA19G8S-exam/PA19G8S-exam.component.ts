...
import * as problemsData from "src/assets/problems/PA19G8S/PA19G8S-problems.json";
...
@Component({
    selector: 'app-PA19G8S-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA19G8S-exam.component.css']
})

@Injectable()
export class PA19G8SExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA19G8S_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
