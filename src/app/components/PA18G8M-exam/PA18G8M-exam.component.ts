...
import * as problemsData from "src/assets/problems/PA18G8M/PA18G8M-problems.json";
...
@Component({
    selector: 'app-PA18G8M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA18G8M-exam.component.css']
})

@Injectable()
export class PA18G8MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA18G8M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
