...
import * as problemsData from "src/assets/problems/TX18G8S/TX18G8S-problems.json";
...
@Component({
    selector: 'app-TX18G8S-exam',
    templateUrl: './TX18G8S-exam.component.html',
    styleUrls: ['./TX18G8S-exam.component.css']
})

@Injectable()
export class TX18G8SExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX18G8S_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
    ...
    completeExam() {
        ...
        else if (this.number_correct >= 30) {
