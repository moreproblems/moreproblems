...
import * as problemsData from "src/assets/problems/TX21G5M/TX21G5M-problems.json";
...
@Component({
    selector: 'app-TX21G5M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX21G5M-exam.component.css']
})

@Injectable()
export class TX21G5MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX21G5M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
