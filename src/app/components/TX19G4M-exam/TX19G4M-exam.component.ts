...
import * as problemsData from "src/assets/problems/TX19G4M/TX19G4M-problems.json";
...
@Component({
    selector: 'app-TX19G4M-exam',
    templateUrl: './TX19G4M-exam.component.html',
    styleUrls: ['./TX19G4M-exam.component.css']
})

@Injectable()
export class TX19G4MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX19G4M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
