...
import * as problemsData from "src/assets/problems/TX17G6M/TX17G6M-problems.json";
...
@Component({
    selector: 'app-TX17G6M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX17G6M-exam.component.css']
})

@Injectable()
export class TX17G6MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX17G6M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
    ...
    completeExam() {
        ...
        else if (this.number_correct >= 13) {
