...
import * as problemsData from "src/assets/problems/PA21G7M/PA21G7M-problems.json";
...
@Component({
    selector: 'app-PA21G7M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA21G7M-exam.component.css']
})

@Injectable()
export class PA21G7MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA21G7M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
