...
import * as problemsData from "src/assets/problems/TX21G8S/TX21G8S-problems.json";
...
@Component({
    selector: 'app-TX21G8S-exam',
    templateUrl: './TX21G8S-exam.component.html',
    styleUrls: ['./TX21G8S-exam.component.css']
})

@Injectable()
export class TX21G8SExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX21G8S_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
