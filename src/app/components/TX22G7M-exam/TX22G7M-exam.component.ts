...
import * as problemsData from "src/assets/problems/TX22G7M/TX22G7M-problems.json";
...
@Component({
    selector: 'app-TX22G7M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX22G7M-exam.component.css']
})

@Injectable()
export class TX22G7MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX22G7M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
