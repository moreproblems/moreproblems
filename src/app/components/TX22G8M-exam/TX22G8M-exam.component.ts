...
import * as problemsData from "src/assets/problems/TX22G8M/TX22G8M-problems.json";
...
@Component({
    selector: 'app-TX22G8M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX22G8M-exam.component.css']
})

@Injectable()
export class TX22G8MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX22G8M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
