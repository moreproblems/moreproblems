...
import * as problemsData from "src/assets/problems/TX19G5M/TX19G5M-problems.json";
...
@Component({
    selector: 'app-TX19G5M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX19G5M-exam.component.css']
})

@Injectable()
export class TX19G5MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX19G5M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
