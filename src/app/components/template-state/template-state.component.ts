...
export class TemplateStateComponent implements OnInit {
  ...
  select_grade(grade: string) {
    ...
    for (const [id, dump] of Object.entries(this.dumpService.standards_attribute_dump)) {
      if (id.endsWith('-' + this.state_key) && this.dumpService.standards_attribute_dump[id].Grades.includes(grade)) {
    ...
    for (const [id, dump] of Object.entries(this.dumpService.exam_attribute_dump)) {
      if (dump.State == this.state_labels[this.state_key] && (this.dumpService.exam_attribute_dump[id].Grade == this.selected_grade || (this.dumpService.exam_attribute_dump[id].Grade == 'High School') && ['Grade 9', 'Grade 10', 'Grade 11', 'Grade 12'].includes(this.selected_grade))) {
  ...
  select_subject(subject: string) {
    ...
    for (const [id, dump] of Object.entries(this.dumpService.standards_attribute_dump)) {
      for (const [name, labels] of Object.entries(this.subject_map)) {
        if (id.endsWith('-' + this.state_key) && this.dumpService.standards_attribute_dump[id].Grades.includes(this.selected_grade) && labels.includes(this.dumpService.standards_attribute_dump[id].Subject) && name == this.selected_subject) {
  ...
  ngOnInit() {
    ...
    this.sub = this.aRoute.paramMap.subscribe((params) => {
      ...
      if (!this.dumpService.state_set.includes(this.state_key)) {
    ...
    //   }
    // }
    // for (let set of Object.keys(this.dumpService.standards_attribute_dump)) {
    //   if (!subject_list.includes(this.dumpService.standards_attribute_dump[set].Subject)) {
    //     subject_list.push(this.dumpService.standards_attribute_dump[set].Subject)
    //   }
    // }
