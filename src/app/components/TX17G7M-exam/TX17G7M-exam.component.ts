...
import * as problemsData from "src/assets/problems/TX17G7M/TX17G7M-problems.json";
...
@Component({
    selector: 'app-TX17G7M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX17G7M-exam.component.css']
})

@Injectable()
export class TX17G7MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX17G7M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
