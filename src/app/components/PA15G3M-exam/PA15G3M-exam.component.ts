...
import * as problemsData from "src/assets/problems/PA15G3M/PA15G3M-problems.json";
...
@Component({
    selector: 'app-PA15G3M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA15G3M-exam.component.css']
})

@Injectable()
export class PA15G3MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA15G3M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
