...
import * as problemsData from "src/assets/problems/TX19G8S/TX19G8S-problems.json";
...
@Component({
    selector: 'app-TX19G8S-exam',
    templateUrl: './TX19G8S-exam.component.html',
    styleUrls: ['./TX19G8S-exam.component.css']
})

@Injectable()
export class TX19G8SExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX19G8S_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
