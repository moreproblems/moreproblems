...
import * as problemsData from "src/assets/problems/PA19G4M/PA19G4M-problems.json";
...
@Component({
    selector: 'app-PA19G4M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA19G4M-exam.component.css']
})

@Injectable()
export class PA19G4MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA19G4M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
