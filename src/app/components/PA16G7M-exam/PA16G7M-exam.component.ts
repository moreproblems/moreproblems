...
import * as problemsData from "src/assets/problems/PA16G7M/PA16G7M-problems.json";
...
@Component({
    selector: 'app-PA16G7M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA16G7M-exam.component.css']
})

@Injectable()
export class PA16G7MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA16G7M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
