...
import * as problemsData from "src/assets/problems/TX22G8SS/TX22G8SS-problems.json";
...
@Component({
    selector: 'app-TX22G8SS-exam',
    templateUrl: './TX22G8SS-exam.component.html',
    styleUrls: ['./TX22G8SS-exam.component.css']
})

@Injectable()
export class TX22G8SSExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX22G8SS_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
