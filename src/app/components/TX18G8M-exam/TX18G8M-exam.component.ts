...
import * as problemsData from "src/assets/problems/TX18G8M/TX18G8M-problems.json";
...
@Component({
    selector: 'app-TX18G8M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./TX18G8M-exam.component.css']
})

@Injectable()
export class TX18G8MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX18G8M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
