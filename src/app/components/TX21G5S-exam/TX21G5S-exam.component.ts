...
@Component({
    ...
    styleUrls: ['./TX21G5S-exam.component.css']
})

@Injectable()
export class TX21G5SExamComponent implements OnInit {
    title = 'More Problems';
    ...
    TX21G5S_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'ProcessSubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
    ...
    completeExam() {
        ...
        else if (this.number_correct >= 21) {
