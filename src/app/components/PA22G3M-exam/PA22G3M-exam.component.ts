...
import * as problemsData from "src/assets/problems/PA22G3M/PA22G3M-problems.json";
...
@Component({
    selector: 'app-PA22G3M-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA22G3M-exam.component.css']
})

@Injectable()
export class PA22G3MExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA22G3M_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
