...
import * as problemsData from "src/assets/problems/PA22G4S/PA22G4S-problems.json";
...
@Component({
    selector: 'app-PA22G4S-exam',
    templateUrl: '../template-exam/template-exam.component.html',
    styleUrls: ['./PA22G4S-exam.component.css']
})

@Injectable()
export class PA22G4SExamComponent implements OnInit {
    title = 'More Problems';
    ...
    PA22G4S_exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topics': string[], 'SubTopics': string[], 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string, 'Percent': number } } } } } = problemsData;
