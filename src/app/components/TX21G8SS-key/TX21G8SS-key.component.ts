...
import * as problemsData from "src/assets/problems/TX21G8SS/TX21G8SS-problems.json";
...
@Component({
    selector: 'app-TX21G8SS-key',
    templateUrl: './TX21G8SS-key.component.html',
    styleUrls: ['./TX21G8SS-key.component.css']
})

@Injectable()
export class TX21G8SSKeyComponent implements OnInit {
    title = 'More Problems';
    ...
    exam_dump: { [key: number]: { 'Number': number, 'Type': string, 'NumChoices': number, 'Topic': string, 'SubTopic': string, 'ProcessSubTopic': string, 'Content': string[], 'AnswerChoices': { [key: string]: { 'Choice': string, 'Key': { 'Correct': boolean, 'Rationale': string } } } } } = problemsData;
